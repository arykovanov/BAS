...
void setDispExit(void) { SoDisp_setExit(dispatcher); }
...
static int tryTraceFile(const char *name) {
  logFp = fopen(name, "wb");
  if (logFp) {
    time_t t = time(0);
...
static void serverErrHandler(BaFatalErrorCodes ecode1, unsigned int ecode2,
...
#ifdef MAKO_HOME_DIR
static void setHomeIoPath(const char *homedir) {
  int status;
  static union {
    DiskIo dio;
    NetIo nio;
...
static int findFlag(int argc, char **argv, const char flag, const char **data) {
  int i;
  for (i = 1; i < argc; i++) {
    char *ptr = argv[i];
...
#ifdef _WIN32
#define BA_WIN32
static int pxexit(int status, int pause) {
  if (pause) {
...
static char *findExecPath(const char *argv0) {
  char buf[2048];
  (void)argv0;
...
static BOOL WINAPI sigTerm(DWORD x) {
  int i;
  makoprintf(FALSE, "\nExiting...\n");
  setDispExit();
...
static void setCtrlCHandler(void) { SetConsoleCtrlHandler(sigTerm, TRUE); }
...
static void setCtrlCHandler(void) {
  signal(SIGINT, sigTerm);
  signal(SIGTERM, sigTerm);
}

static void cfignoreSignal(int sig, const char *signame) {
  struct sigaction sa;
...
#ifndef BA_VXWORKS
static void cfblockSignal(int sig, const char *signame) {
  sigset_t set;
  sigemptyset(&set);
  sigaddset(&set, sig);
...
static void disableSignals(void) { ignoreSignal(SIGPIPE); }

#ifdef BA_VXWORKS
#define setLinuxDaemonMode()
#else
static void setLinuxDaemonMode(void) {
  int fd0, fd1, fd2;
  pid_t pid;
...
#define setUser(x, y)
#else
static void setUser(int argc, char **argv) {
  const char *userName;
 ...
 */
static void onunload(int onunloadRef) {
...
static int loadConfigData(lua_State *L) {
  U16 val;
  MakoServerConfig *mcfg = (MakoServerConfig *)lua_touserdata(L, 1);
#ifdef BA_VXWORKS
  int status = balua_loadfile(L, (const char *)lua_touserdata(L, 2),
                              (IoIntf *)&diskIo, 0);
#else
  ...
  *paramPtr = "commandcnt";
  ...
  HttpServerConfig_setNoOfHttpCommands(&mcfg->scfg,
                                       (U16)luaL_optinteger(L, -1, 3));
  ...
  *paramPtr = "conncnt";
  lua_getglobal(L, *paramPtr);
  HttpServerConfig_setNoOfHttpConnections(&mcfg->scfg,
                                          (U16)luaL_optinteger(L, -1, 50));

  *paramPtr = "rspsz";
  lua_getglobal(L, *paramPtr);
  HttpServerConfig_setResponseData(&mcfg->scfg,
                                   (U16)luaL_optinteger(L, -1, 8 * 1024));
  ...
  *paramPtr = "reqmaxsz";
  lua_getglobal(L, *paramPtr);
  HttpServerConfig_setRequest(&mcfg->scfg, val,
                              (U16)luaL_optinteger(L, -1, 8 * 1024));
...
static char *openAndLoadConf(const char *path, MakoServerConfig *mcfg,
                             int isdir) {
  char *name = 0;
  char *abspath = 0;
  if (path) {
...
static char *openAndLoadConfUsingExecPath(MakoServerConfig *cfg,
                                          const char *argv0, char **execpath) {
...
*/
static char *createServer(int argc, char **argv, char **execpath) {
...
#else
  BaTimer_constructor(&timer, &mutex, 10000, mcfg.timerTick, ThreadPrioNormal,
                      0);
#endif
...
static int mako_exit(lua_State *L) { exit((int)luaL_optinteger(L, 1, 0)); }

/* Create Lua global variables "argv" for the command line arguments and
 * "env" for the system environment variables.
 */
static void createLuaGlobals(int argc, char **argv, char **envp, char *cfgfname,
                             const char *execpath) {
  int i;
  char *ptr;
  char buf[200];
  static const luaL_Reg funcs[] = {{"tracefile", lSetTraceFile}, {NULL, NULL}};
...
#else
      " -d                       - Run in daemon mode by detatching from the "
      "console\n"
      " -s                       - Run in daemon mode without detatching from "
      "the console\n"
...
static ZipIo *createAndCheckMakoZipIo(ZipReader *zipReader, const char *name) {
...
static IoIntf *openMakoZip(const U8 *zipPubKey, IoIntf *rootIo,
                           const char *path) {
  IoStat st;
  ZipIo *io = 0;
  int plen = iStrlen(path);
  char *buf = (char *)baMalloc(plen + 12);
  basprintf(buf, "%s%smako.zip", path,
            path[plen - 1] == *LUA_DIRSEP ? "" : LUA_DIRSEP);
  if (*LUA_DIRSEP == '\\') {
    char *ptr = buf;
    while (*ptr) {
      if (*ptr == '\\')
  ...
  if (!rootIo->statFp(rootIo, buf, &st)) {
    ...
    L_err:
      ...
      makoZipReader = 0;
               ...
               baCheckZipSignature(zipPubKey, makoZipReader->reader.size,
                                   (CspReader *)makoZipReader)) {
      ZipIo_destructor(io);
      baFree(io);
      io = 0;
      goto L_err;
...
static IoIntf *openVmIo(const U8 *zipPubKey, IoIntf *rootIo,
                        const char *execpath, const char *cfgfname,
                        BaBool *limitedZip) {
  IoIntf *io = 0;
  char *path;
  ...
  */
  {
    ...
    io = (IoIntf *)createAndCheckMakoZipIo(
        getLspZipReader(), "'the limited embedded mako.zip'. "
 ...
 */
static void addSecret(MakoOpenAUX *aux, BaBool unique, const U8 *secret,
                      size_t slen) {
  lua_State *L = aux->L;
...
#include "MakoExtM1.ch" /* Inject optional code */
    ...
    for (i = 1; i < argc; i++) {
      ...
      if (*ptr == '-') {
        ...
        if (!strcmp("autostart", ptr)) {
          ...
          mustBeAdmin();
          pxexit(prtok(serviceSetStartup(SERVICENAME, SERVICE_DEMAND_START) ==
          ...
          if (serviceState(SERVICENAME) != SERVICE_STOPPED) {
            mustBeAdmin();
            pxexit(
                prtok(serviceStop(SERVICENAME) == SERVICE_STOPPED, "stopped"),
                TRUE);
          } else
            pxexit(1, TRUE);
  ...
  if (!isWinService && sMode) {
    ...
    mustBeAdmin();
    if (sMode == 3) {
      status = prtok(serviceStart(SERVICENAME, argc, argv) == SERVICE_RUNNING,
                     "started");
  ...
  balua_usertracker_create(L, cfgParams->trackerNodes, cfgParams->maxlogin,
                           cfgParams->bantime);
  baFree(cfgParams);
  ...
  LThreadMgr_constructor(&ltMgr, /* Install optional Lua thread library */
                         &server, ThreadPrioNormal, BA_STACKSZ,
                         3,        /* Number of threads */
                         L, TRUE); /* allow creating more threads */
